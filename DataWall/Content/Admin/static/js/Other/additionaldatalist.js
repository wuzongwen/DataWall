layui.use(['laydate', 'form', 'jquery', 'admin', 'upload'], function () {
    ...
    var uploadInst = upload.render({
        ...
        , url: '/Other/UploadExcel?Type=' + $('#Type').val() //上传接口
        , before: function (obj) {
            //加载层-风格4
            layer.msg('数据导入中', {
                icon: 16
    ...
    window.member_stop = function (obj, id) {
        ...
        layer.confirm('确认要' + title + '吗？', function (index) {
            ...
            $.ajax({
                type: 'post',
                url: '/Other/AdditionalDataEditEnable',
                dataType: 'json',
                data: {
                    id: id,
                    enable: enable
                    ...
                    switch (code) {
                        case '200':
                            layer.alert(msg.msg, { icon: 1 }, function () {
                                location.href = "/Other/AdditionalDataList?page=" + msg.page + "&Type=" + type;
                            });
                            break;
                        case '201':
                            layer.msg(msg.msg);
    ...
    window.delAll = function (type, page) {
        ...
        if (data.length > 0) {
            layer.confirm('确认要删除吗？', function (index) {
                ...
                $.ajax({
                    type: 'post',
                    url: '/Other/DelAdditionalDataAll',
                    dataType: 'json',
                    data: {
                        idList: String(data),
                        page: page
